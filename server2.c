...
int findTokens(char *buffer, struct _tokens *tokens)
{
  ...
  while (ptr != NULL) // keep going until no more ' '
  {
    ...
    int flag = 0; //
    int k = 0;
    memset(tokens[counter].key, 0, 10000);
    memset(tokens[counter].value, 0, 10000);

    for (i = 0; i < strlen(ptr); i++)
    {
      if (flag == 0)
      { // doing the key portion
        if (ptr[i] != ':')
      ...
      { // doing the value portion
        if (ptr[i] == '^')
        { // undoing the cleansing of msg portion
          ptr[i] = ' ';
        }
...
void receive_data(int sd, int portNumber)
{
  ...
  // Check if the "port" key exists in the message and if its value matches the server's port number
  int valid_port = 0;
  for (int i = 0; i < num_tokens; i++)
  {
    if (strcmp(tokens[i].key, "port") == 0)
    {
      int port_value = atoi(tokens[i].value);
      if (port_value == portNumber)
      {
        valid_port = 1;
        break;
      }
  ...
  if (!valid_port)
  {
    printf("Received a message not destined for this server (port %d)\n", portNumber);
    return;
  }
...
int main(int argc, char *argv[])
{
  ...
  while (1)
  {
    ...
    int is_for_this_server = 0;
    for (int i = 0; i < num_tokens; i++)
    {
      if (strcmp(tokens[i].key, "port") == 0 && atoi(tokens[i].value) == portNumber)
      {
        is_for_this_server = 1;
        break;
      }
    }

    // Debugging print statement
    printf("Is message for this server? %d\n", is_for_this_server);

    // Print message only if it is for this server
    if (is_for_this_server)
    {
      printf("Received data from %s:%d\n", inet_ntoa(client_address.sin_addr), ntohs(client_address.sin_port));
      printf("Message: %s\n", bufferIn);
    ...
    {
      printf("Received message not destined for this server.\n");
    }
  }
